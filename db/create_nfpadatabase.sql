--
-- File generated with SQLiteStudio v3.1.0 on Fri Sep 30 13:17:40 2016
--
...
CREATE TABLE measurements (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, ts DOUBLE NOT NULL, name TEXT, cpu INTEGER NOT NULL REFERENCES cpu (id) ON DELETE CASCADE ON UPDATE CASCADE, nic INTEGER NOT NULL REFERENCES nic (id) ON DELETE CASCADE ON UPDATE CASCADE, virtualization INTEGER NOT NULL REFERENCES virtualization (id) ON DELETE CASCADE ON UPDATE CASCADE, vnf INTEGER NOT NULL REFERENCES vnf (id) ON DELETE CASCADE ON UPDATE CASCADE, used_cpu_cores INTEGER NOT NULL CONSTRAINT default_used_cpu_cores DEFAULT (1), traffic INTEGER NOT NULL REFERENCES traffic_names (id) ON DELETE CASCADE ON UPDATE CASCADE, repetitions INTEGER NOT NULL, duration INTEGER NOT NULL, sent_pps_min BIGINT NOT NULL, sent_pps_avg DOUBLE NOT NULL, sent_pps_max BIGINT NOT NULL, recv_pps_min BIGINT NOT NULL, recv_pps_avg DOUBLE NOT NULL, recv_pps_max BIGINT NOT NULL, miss_pps_min BIGINT NOT NULL, miss_pps_avg DOUBLE NOT NULL, miss_pps_max BIGINT NOT NULL, sent_bps_min BIGINT NOT NULL, sent_bps_avg DOUBLE NOT NULL, sent_bps_max BIGINT NOT NULL, recv_bps_min BIGINT NOT NULL, recv_bps_avg DOUBLE NOT NULL, recv_bps_max BIGINT NOT NULL, diff_bps_min BIGINT NOT NULL, diff_bps_avg DOUBLE NOT NULL, diff_bps_max BIGINT NOT NULL, bidir_twin_id INTEGER, comment TEXT, user_id INTEGER REFERENCES users (id) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL, bidir BOOLEAN NOT NULL DEFAULT FALSE, FOREIGN KEY (cpu) REFERENCES cpu_makes (id), FOREIGN KEY (nic) REFERENCES nic (id), FOREIGN KEY (virtualization) REFERENCES virtualization (id), FOREIGN KEY (vnf) REFERENCES vnf (id), FOREIGN KEY (traffic) REFERENCES traffic_names (id));
...
CREATE VIEW measurements_view AS SELECT measurements.id,
           measurements.name,
           ...
           measurements.used_cpu_cores as num_cores,
...
CREATE VIEW measurements_view_v2 AS SELECT measurements.id,
           measurements.name,
           traffic_view.name AS traffic_name,
           traffic_view.packet_size AS traffic_packet_size,
           measurements.recv_pps_avg,
