...
function start_measurement ()
  ...
  do
    ...
    -- set packet size in var size
    size = pktSizes[i];
    

    -- print("FILE CHECK");
    -- check outputfile existance    
    file_exists=file_check("nfpa.simple.".. size .. "bytes.res");
    
    
    -- create file descriptor for output
    local file = io.open("nfpa.simple.".. size .. "bytes.res","a");

    -- set default output file to the created file descriptor
    io.output(file);
    
    
    -- print("Start measurement with " .. size .. " packets\n");
    pktgen.set(tonumber(config["sendPort"]), "size", size);
    pktgen.start(tonumber(config["sendPort"]));
    
    -- set the other port as well if biDir is set
    if(tonumber(config["biDir"]) == 1)
    then
      -- set packet size for the other port as well
      pktgen.set(tonumber(config["recvPort"]), "size", size);
    ...
    if(tonumber(config["biDir"]) == 1)
    then
      print("Bi-directional measurement started with " .. size .. 
            " bytes packets\n");
    else
      print("Measurement started with " .. size .. " bytes packets\n");      
    end  
    ...
    do
      ...
      then
        ...
        -- calculate sending throughput in mbps
        sent_bps2 = math.floor(sent_pkts2*(size+20)*8);
        -- get received packet/s data
        recv_pkts2 = portRates[tonumber(config["sendPort"])].pkts_rx;
        -- calculate receiving throughput in mbps    
        recv_bps2 = math.floor(recv_pkts2*(size+20)*8);  
        -- calculate the number of missing packets
