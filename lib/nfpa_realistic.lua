...
-- TODO: getting throughput in BPS!
...
function start_measurement ()
  ...
  then
    ...
    print("Start traffic on port " .. config["recvPort"]);
  ...
  do
    ...
    then
      ...
      -- we need to rely on pktgen's calculation for getting throughput in bps
      sent_bps2 = portRates[tonumber(config["recvPort"])].mbits_tx;
      -- since it is in mbit/s we change it to bps
      sent_bps2 = sent_bps2 * 1000000;
      -- get received packet/s data
      recv_pkts2 = portRates[tonumber(config["sendPort"])].pkts_rx;
      -- we need to rely on pktgen's calculation for getting throughput in bps
      recv_bps2 = portRates[tonumber(config["sendPort"])].mbits_rx;
      -- since it is in mbit/s we change it to bps
      recv_bps2 = recv_bps2 * 1000000;
      -- calculate the number of missing packets
